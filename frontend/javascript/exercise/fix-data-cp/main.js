 ...
 * line = 'abcabdueobbb'
 *
 * Karena tidak terdapat virus, maka output yang diharapkan adalah 'abcabdueobbb'
 *
 * [Ketentuan]: - TIDAK BOLEH menggunakan built-in function!
 */
...
function fixData(line) {
  ...
  } else if (hitungKonsonan > hitungVokal) {
    for (var i = 0; i < line.length; i++) {
      if (line[i] === "#") {
        data += "a";
      } else {
  ...
  } else if (hitungVokal === hitungKonsonan) {
    for (var i = 0; i < line.length; i++) {
      if (line[i] === "#") {
        data += "c";
      } else {
