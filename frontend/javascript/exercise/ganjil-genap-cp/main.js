...
function ganjilGenap(plat) {
  ...
  if (plat === undefined)
    ...
    for (let i = 0; i < platStr.length; i++) {
      if (platStr[i] % 2 === 0) {
        numGenap++;
      } else {
        numGanjil++;
    ...
    if (numGanjil === 0 && numGenap === 0)
      return "plat ganjil tidak ditemukan dan plat genap tidak ditemukan";
    else if (numGanjil === 0)
      return `plat genap sebanyak ${numGenap} dan plat ganjil tidak ditemukan`;
    else if (numGenap === 0)
      return `plat ganjil sebanyak ${numGanjil} dan plat genap tidak ditemukan`;
    else
      return `plat genap sebanyak ${numGenap} dan plat ganjil sebanyak ${numGanjil}`;
}
