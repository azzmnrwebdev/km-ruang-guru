...
// TODO: answer here
maxSum = arr.reduce((a, b) => Math.max(a, b)); // reduce adalah fungsi untuk menjumlahkan array
if (arr.length === 0) {
  maxSum = 0;
} else if (arr.every((num) => num < 0)) {
  // every adalah fungsi untuk mengecek apakah semua elemen array sesuai dengan kondisi
  maxSum = 0;
} else {
  let max = arr[0];
